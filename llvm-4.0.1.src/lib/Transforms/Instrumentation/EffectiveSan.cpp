...
#define TYPE_TID_INFO_PREFIX "EFFECTIVE_TID_"
...
const my_effective_info MY_EFFECTIVE_INFO_FREE = {.global_name =
                                                      "EFFECTIVE_INFO_FREE",
                                                  .tid = 0,
                                                  .num_access = 0,
                                                  .name = "<free memory>",
...
const my_effective_info MY_EFFECTIVE_INFO_INT8_PTR = {
    .global_name = "EFFECTIVE_INFO_INT8_PTR",
    .tid = 1,
    .num_access = 0,
    .name = "int8_t *",
    .size = sizeof(void *),
...
const my_effective_info MY_EFFECTIVE_INFO_INT8 = {.global_name =
                                                      "EFFECTIVE_INFO_INT8",
                                                  .tid = 2,
                                                  .num_access = 0,
                                                  .name = "int8_t",
...
const my_effective_info MY_EFFECTIVE_INFO_INT16 = {.global_name =
                                                       "EFFECTIVE_INFO_INT16",
                                                   .tid = 3,
                                                   .num_access = 0,
                                                   .name = "int16_t",
                                                   .size = sizeof(int16_t),
...
const my_effective_info MY_EFFECTIVE_INFO_INT32 = {.global_name =
                                                       "EFFECTIVE_INFO_INT32",
                                                   .tid = 4,
                                                   .num_access = 0,
                                                   .name = "int32_t",
                                                   .size = sizeof(int32_t),
...
const my_effective_info MY_EFFECTIVE_INFO_INT64 = {.global_name =
                                                       "EFFECTIVE_INFO_INT64",
                                                   .tid = 5,
                                                   .num_access = 0,
                                                   .name = "int64_t",
                                                   .size = sizeof(int64_t),
...
const my_effective_info MY_EFFECTIVE_INFO_INT128 = {.global_name =
                                                        "EFFECTIVE_INFO_INT128",
                                                    .tid = 6,
                                                    .num_access = 0,
                                                    .name = "int128_t",
...
const my_effective_info MY_EFFECTIVE_INFO_FLOAT16 = {
    .global_name = "EFFECTIVE_INFO_FLOAT16",
    .tid = 7,
    .num_access = 0,
    .name = "float16_t",
    .size = 2,
...
const my_effective_info MY_EFFECTIVE_INFO_FLOAT32 = {
    .global_name = "EFFECTIVE_INFO_FLOAT32",
    .tid = 8,
    .num_access = 0,
    .name = "float32_t",
    .size = sizeof(float),
...
const my_effective_info MY_EFFECTIVE_INFO_FLOAT64 = {
    .global_name = "EFFECTIVE_INFO_FLOAT64",
    .tid = 9,
    .num_access = 0,
    .name = "float64_t",
    .size = sizeof(double),
...
const my_effective_info MY_EFFECTIVE_INFO_FLOAT80 = {
    .global_name = "EFFECTIVE_INFO_FLOAT80",
    .tid = 10,
    .num_access = 0,
    .name = "float80_t",
    .size = 12,
...
const my_effective_info MY_EFFECTIVE_INFO_FLOAT128 = {
    .global_name = "EFFECTIVE_INFO_FLOAT128",
    .tid = 11,
    .num_access = 0,
    .name = "float128_t",
    .size = 16,
...
std::string hashFileName = "type_tree.hash";
std::map<std::string, my_effective_info> basicTypes;
uint64_t currentTypeID;
...
//static llvm::StructType *TyCheSectionEntryTy = nullptr;
...
typedef std::map<llvm::Value *, std::vector<BoundsCheckEntry>> BoundsCheckInfo;
...
static llvm::cl::opt<bool>
    option_no_escapes("effective-no-escapes",
                      llvm::cl::desc("Do not instrument pointer escapes"));
static llvm::cl::opt<bool>
    option_no_globals("effective-no-globals",
                      llvm::cl::desc("Do not instrument global variables"));
static llvm::cl::opt<bool>
    option_no_stack("effective-no-stack",
                    llvm::cl::desc("Do not instrument stack objects"));
static llvm::cl::opt<std::string> option_blacklist(
    "effective-blacklist",
    llvm::cl::desc("Do not instrument code based on the given blacklist"),
    llvm::cl::init("-"));
static llvm::cl::opt<bool>
    option_warnings("effective-warnings",
                    llvm::cl::desc("Enable warning messages"));
static llvm::cl::opt<unsigned> option_max_sub_objs(
    "effective-max-sub-objs",
...
calculateBoundsConstant(const llvm::DataLayout &DL, llvm::Constant *Ptr);
...
static bool isAnonymousType(llvm::DIType *Ty) {
  ...
  if (ident.size() >= 4 && ident[0] == '_' && ident[1] == 'Z' &&
    ...
    int status = 0;
    char *res = abi::__cxa_demangle(ident.str().c_str(), NULL, NULL, &status);
    if (status != 0)
      return false;
...
public:
  ...
  static void init() { DK = llvm::getNextAvailablePluginDiagnosticKind(); }
};
...
static llvm::StructType *makeTypeMetaType(llvm::Module &M, size_t len, llvm::ArrayType * tyche_cl_type) {
  ...
  Fields.push_back(llvm::Type::getInt32Ty(Cxt)); /* length */
...
static llvm::StructType *makeTyCheCacheLineType(llvm::Module &M, int64_t tid, size_t offset, size_t section) {


  llvm::LLVMContext &Cxt = M.getContext();

  std::string name("TYCHE_META_CACHELINE");
  if (tid >= 0) {
    name += std::to_string(tid) + std::to_string(offset) + std::to_string(section);
  }

  llvm::StructType *Ty = llvm::StructType::create(Cxt, name);

  if (tid == -1)
    TyCheCacheLineEntryTy = Ty;


  std::vector<llvm::Type *> Fields;
  for (size_t i = 0; i < NUMBER_OF_ENTRIES_IN_EACH_CACHELINE; i++)
  {
    Fields.push_back(llvm::Type::getInt32Ty(Cxt)); /* Meta #n */
  }      
  Fields.push_back(TyCheCacheLineEntryTy->getPointerTo());        /* next cacheline */
  Ty->setBody(Fields, true);
...
static llvm::DIType *getMemberType(llvm::DIType *Ty, size_t &offset,
                                   bool &isStatic, bool &isBitField,
                                   bool &isInheritance, bool &isVirtual) {
  while (true) {
    if (isVPtrType(Ty)) {
      Ty = Int8PtrTy;
      break;
    ...
    case llvm::dwarf::DW_TAG_member:
      if (DerivedTy->isBitField()) {
        ...
        llvm::Constant *Offset = DerivedTy->getStorageOffsetInBits();
        if (Offset != nullptr && llvm::isa<llvm::ConstantInt>(Offset)) {
...
static void buildLayout(llvm::DICompositeType *CompositeTy, size_t offsetBase,
  ...
  for (auto Element : Elements) {
    ...
    TyCheDependencyTree.push_back(llvm::dyn_cast<llvm::DIType>(Element));
    buildMemberLayout(Ty, offset, fam, lb, ub, priority,
                      (inherited || isInheritance), tInfo, layout, tyCheEntry, TyCheDependencyTree);
...
static void buildMemberLayout(llvm::DIType *Ty, size_t offset, const FAM &fam,
  ...
  if (offset == fam.offset && Ty == fam.type) {
    ...
    if (CompositeTy != nullptr)
    {
      TyCheDependencyTree.push_back(CompositeTy);
      buildLayout(CompositeTy, offset, fam, tInfo, layout, TyCheDependencyTree,
...
static size_t getLayoutLength(const LayoutInfo &layout) {
  ...
  if (full > UB) {
    EFFECTIVE_DEBUG_PRINT("warning: sub-optimal hash table size: "
                          "[%zu ---> %zu] (%.2f%%)\n",
                          len, len2, ((double)len / (double)len2) * 100.0);
  }
  return len2;
}
...
static bool placeFlattenedLayoutEntry(FlattenedLayoutInfo &flattenedLayout,
                                      uint64_t hval1, size_t offset,
                                      uint64_t mask, LayoutEntry &lEntry) {
  if (lEntry.deleted)
    return true;
  ...
  for (i = 0; i < EFFECTIVE_MAX_PROBE; i++) {
    ...
    if (j == flattenedLayout.end()) {
      ...
      flattenedLayout.insert(std::make_pair(eidx, &lEntry));
      return placeFlattenedLayoutEntry(flattenedLayout, hval1, oldEntry->offset,
                                       mask, *oldEntry);
    }
    if (&lEntry == j->second) {
...
static int64_t compileLayoutToFlattenLayoutForTyChe(llvm::Module &M,
  ...
  for (auto &entries : flattenedLayout) {
      OffsetSoretedFlattenedLayoutInfo.insert(std::make_pair((uint32_t)entries.second->offset, entries.second));
  }
  ...
  for (auto &entries : OffsetSoretedFlattenedLayoutInfo)
  {
      ...
      meta_offset = entries.first / TYCHE_OFFSETS_DEVIDER;
      ...
      llvm::Constant *Entry =  llvm::ConstantInt::get(llvm::Type::getInt32Ty(Cxt), entries.first);
      total_offset += entries.first;
      total_name += entries.second->humanName;

      if (TyCheMetaCacheLinesSections[TypeId][meta_offset][section_number].size() < NUMBER_OF_ENTRIES_IN_EACH_CACHELINE)
      {
          TyCheMetaCacheLinesSections[TypeId][meta_offset][section_number].push_back(Entry);
      }
      else 
      {
          assert(TyCheMetaCacheLinesSections[TypeId][meta_offset][section_number].size() == NUMBER_OF_ENTRIES_IN_EACH_CACHELINE);
          section_number++;
...
static llvm::Constant* getTyCheMeta(llvm::Module &M, uint64_t tid, llvm::ArrayType* &tyche_cl_meta_type)
{
      llvm::LLVMContext &Cxt = M.getContext();
      ...
      std::vector<std::vector<std::vector<llvm::Constant *>>> SectionEntries(TYCHE_NUMBER_OF_SECTIONS);
      std::vector<std::vector<bool>> SectionStates(TYCHE_NUMBER_OF_SECTIONS);
      ...
      for (size_t offset = 0; offset < TyCheMetaCacheLinesSections[tid].size(); offset++)
      {
        ...
        //llvm::Constant * PrevSectionTyCheCacheLineGV = nullptr;
        assert(TyCheMetaCacheLinesSections[tid][offset].size() > 0);
        for (int section = (int)TyCheMetaCacheLinesSections[tid][offset].size() - 1; section >= 0 ; section--)
        {
          assert(TyCheMetaCacheLinesSections[tid][offset].find(section) != TyCheMetaCacheLinesSections[tid][offset].end());
          assert(TyCheMetaCacheLinesSections[tid][offset][section].size() <= NUMBER_OF_ENTRIES_IN_EACH_CACHELINE);
          ...
          assert(TyCheMetaCacheLinesSections[tid][offset][section].size() == NUMBER_OF_ENTRIES_IN_EACH_CACHELINE);
          SectionEntries[section].push_back(TyCheMetaCacheLinesSections[tid][offset][section]);
          
          // // if this is the last section, add a nullptr to it struct
        ...
        // fill empty sections with nullptr in order to keep cachelines in the same Meta Cache set
        for (size_t section = TyCheMetaCacheLinesSections[tid][offset].size(); section < TYCHE_NUMBER_OF_SECTIONS; section++)
        {
      ...
      // Final Step: Emit array of cachelines for this TID into their respective sessions
      assert(SectionEntries.size() == TYCHE_NUMBER_OF_SECTIONS);
      llvm::Constant *TyCheSection_0 = nullptr;
      llvm::GlobalVariable * PrevSectionTyCheCacheLineGV = nullptr;
      llvm::ArrayType * PrevSectionTyCheCacheLineType = nullptr;
      for (int sec = (int)SectionEntries.size() -1; sec >= 0; sec--)
      {
          assert(SectionEntries[sec].size() <= TYCHE_NUMBER_OF_OFFSETS());
          // last section
          if (sec ==  SectionEntries.size() -1)
          {
              
              // for the last section all the pointers are always null 
              ...
              for (size_t off = 0; off < SectionEntries[sec].size(); off++)
              {
                  ...
                  SectionEntries[sec][off].push_back(Entry);

                  llvm::Constant *TyCheCacheLineInit = llvm::ConstantStruct::get(TyCheTy, SectionEntries[sec][off]);
                  SectionConstants.push_back(TyCheCacheLineInit);
              }
              ...
              PrevSectionTyCheCacheLineGV = TyCheSectionMetaGV;
      ...
      // return the section 0 ptr
      assert(tyche_cl_meta_type != nullptr);
      return  TyCheSection_0;
...
static std::pair<llvm::Constant *, size_t> compileLayout(llvm::Module &M,
  ...
  for (auto entries: flattenedLayout ) {
    ...
    Elems.push_back(llvm::ConstantInt::get(llvm::Type::getInt64Ty(Cxt), lEntry.offset));
...
static void buildCompositeTypeName(llvm::DICompositeType *CompositeTy,
                                   std::string &name, TypeInfo &tInfo,
                                   bool demangle = false,
...
static llvm::Constant *buildTypeInfo(llvm::Module &M, llvm::DIType *Ty,
                                     size_t size, const FAM &fam,
                                     bool incomplete, TypeInfo &tInfo) {
  Ty = normalizeType(Ty);
  ...
  llvm::LLVMContext &Cxt = M.getContext();
  llvm::Constant *NameInit = llvm::ConstantDataArray::getString(Cxt, humanName);
  llvm::GlobalVariable *NameGV = new llvm::GlobalVariable(
      M, NameInit->getType(), true, llvm::GlobalValue::PrivateLinkage, NameInit,
      "EFFECTIVE_STRING");
  NameGV->setUnnamedAddr(llvm::GlobalValue::UnnamedAddr::Global);
  llvm::Constant *Name =
      llvm::ConstantExpr::getPointerCast(NameGV, llvm::Type::getInt8PtrTy(Cxt));
  ...
  if (auto *CompositeTy = llvm::dyn_cast<llvm::DICompositeType>(Ty)) {
    ...
    for (auto Element : Elements) {
      ...
      llvm::Constant *Next = buildTypeInfo(M, Ty, size, NONE, false, tInfo);
      llvm::Constant *Flags =
          llvm::ConstantInt::get(llvm::Type::getInt32Ty(Cxt), flags);
      llvm::Constant *LB =
          llvm::ConstantInt::get(llvm::Type::getInt64Ty(Cxt), lb);
      llvm::Constant *UB =
          llvm::ConstantInt::get(llvm::Type::getInt64Ty(Cxt), ub);
      llvm::Constant *Entry =
          llvm::ConstantStruct::get(InfoEntryTy, {Next, Flags, LB, UB});
      Entries.push_back(Entry);
    }
  }
  ...
  llvm::GlobalVariable *InfoGV = new llvm::GlobalVariable(
      M, InfoTy1, true, llvm::GlobalValue::WeakAnyLinkage, 0, infoName.str());
  llvm::Constant *Size =
  ...
  llvm::ArrayType *EntriesTy =
      llvm::ArrayType::get(InfoEntryTy, Entries.size());
  llvm::Constant *Entries1 = llvm::ConstantArray::get(EntriesTy, Entries);
  llvm::Constant *InfoInit = llvm::ConstantStruct::get(
      InfoTy1, {Name, Size, NumEntries, Flags, Next, Entries1});
...
static HashVal buildTypeHash(llvm::DIType *Ty, TypeInfo &tInfo) {
  ...
  if (Ty == nullptr)
    ...
    HashVal hash0 = buildTypeHash(DerivedTy->getBaseType().resolve(), tInfo);
    switch (DerivedTy->getTag()) {
    case llvm::dwarf::DW_TAG_pointer_type:
    case llvm::dwarf::DW_TAG_reference_type:
    case llvm::dwarf::DW_TAG_rvalue_reference_type:
    ...
    switch (CompositeTy->getTag()) {
    case llvm::dwarf::DW_TAG_enumeration_type:
      prefix = ENUM_TYPE_TAG;
      break;
    case llvm::dwarf::DW_TAG_structure_type:
      prefix = STRUCT_TYPE_TAG;
      break;
    case llvm::dwarf::DW_TAG_class_type:
      prefix = CLASS_TYPE_TAG;
      break;
    case llvm::dwarf::DW_TAG_union_type:
      prefix = UNION_TYPE_TAG;
      break;
    default:
...
static FAM getFlexibleArrayMember(llvm::DIType *Ty, size_t baseOffset = 0) {
  ...
  for (auto Element : Elements) {
    ...
    Ty = getMemberType(Ty, offset, isStatic, isBitField, isInheritance,
                       isVirtual);
    if (Ty == nullptr || isStatic || isInheritance || isBitField || isVirtual)
      continue;
    offset = offset / CHAR_BIT;
...
static  TypeEntry &compileType(llvm::Module &M, llvm::DIType *Ty,
  ...
  if (auto *MetaGV = M.getGlobalVariable(metaName.str())) {
    ...
    llvm::Constant *Meta =
        llvm::ConstantExpr::getBitCast(MetaGV, TypeTy->getPointerTo());
    entry.typeMeta = Meta;
    return entry;
  }
  ...
  std::vector<llvm::DIType*> type_dependency; 
  type_dependency.push_back(Ty);
  addLayoutEntry(layout, tInfo, 0, Ty, -EFFECTIVE_DELTA, EFFECTIVE_DELTA, true, tyCheEntry, type_dependency);
  type_dependency.pop_back();
...
#endif
    std::vector<llvm::DIType*> type_dependency;
    type_dependency.push_back(CompositeTy);
    buildLayout(CompositeTy, 0, fam, tInfo, layout, type_dependency);
#ifdef EFFECTIVE_LAYOUT_DEBUG
    fprintf(stderr, "\n");
#endif
  ...
  Elems.push_back(
      llvm::ConstantInt::get(llvm::Type::getInt32Ty(Cxt), size_fam));
  Elems.push_back(
      llvm::ConstantInt::get(llvm::Type::getInt32Ty(Cxt), offset_fam));
  Elems.push_back(
      llvm::ConstantInt::get(llvm::Type::getInt32Ty(Cxt), EFFECTIVE_SANITY));
  Elems.push_back(llvm::ConstantInt::get(llvm::Type::getInt64Ty(Cxt), magic));
  Elems.push_back(llvm::ConstantInt::get(llvm::Type::getInt64Ty(Cxt), mask));
  Elems.push_back(Info);
  Elems.push_back(Next);
  Elems.push_back(llvm::ConstantInt::get(llvm::Type::getInt32Ty(Cxt), finalLen));
...
static InsertPoint nextInsertPoint(llvm::Function &F, llvm::Value *Ptr) {
  if (auto Invoke = llvm::dyn_cast<llvm::InvokeInst>(Ptr)) {
    ...
    return IP;
  } else if (llvm::isa<llvm::Argument>(Ptr) || llvm::isa<llvm::Constant>(Ptr)) {
...
static llvm::DIType *getNewArrayType(llvm::Module &M, llvm::DIType *Ty,
  ...
  buildTypeHumanName(Ty, name, tInfo);
  name += "[]";
  llvm::DIType *CookieTy = builder.createBasicType(
      "unsigned long", sizeof(size_t) * CHAR_BIT, llvm::dwarf::DW_ATE_unsigned);
  CookieTy = builder.createMemberType(Scope, "cookie", File, Line,
                                      CookieTy->getSizeInBits(), 0, 0,
                                      llvm::DINode::FlagPublic, CookieTy);
  ...
  Ty = builder.createMemberType(Scope, "data", File, Line,
                                DataTy->getSizeInBits(),
                                DataTy->getSizeInBits(), cookieSize * CHAR_BIT,
                                llvm::DINode::FlagPublic, DataTy);
  llvm::DINodeArray Elements = builder.getOrCreateArray({CookieTy, Ty});
  size_t size = cookieSize + getSizeOfType(Ty);
  Ty = builder.createStructType(Scope, name, File, Line, size * CHAR_BIT, 0,
                                llvm::DINode::DIFlags(), nullptr, Elements);
      ...
      if (Metadata != nullptr && Metadata->getNumOperands() == 1) {
        ...
        if (MD != nullptr && llvm::isa<llvm::ConstantAsMetadata>(MD)) {
          ...
          if (C != nullptr && llvm::isa<llvm::ConstantInt>(C)) {
            llvm::ConstantInt *Int = llvm::dyn_cast<llvm::ConstantInt>(C);
...
static bool isDerivedPointer(const llvm::DataLayout &DL, llvm::Value *Ptr,
  ...
  if (auto *GEP = llvm::dyn_cast<llvm::GetElementPtrInst>(Ptr)) {
    ...
    if (GEP->accumulateConstantOffset(DL, Offset) && Offset.getSExtValue() == 0)
...
calculateBoundsConstant(const llvm::DataLayout &DL, llvm::Constant *Ptr) {
  ...
  const std::pair<intptr_t, intptr_t> WIDE_BOUNDS = {INTPTR_MIN, INTPTR_MAX};
  ...
  if (auto *CE = llvm::dyn_cast<llvm::ConstantExpr>(Ptr)) {
    ...
    case llvm::Instruction::GetElementPtr: {
      ...
      llvm::Type *Ty = Ptr->getType();
      std::pair<intptr_t, intptr_t> Bounds = calculateBoundsConstant(DL, Ptr);
      ...
      for (int i = 0; i < numIdxs; i++) {
        ...
        if (auto *PtrTy = llvm::dyn_cast<llvm::PointerType>(Ty)) {
          ...
          ssize_t offset =
              (K == nullptr ? 0 : K->getSExtValue() * DL.getTypeAllocSize(Ty));
    ...
    case llvm::Instruction::Select: {
      ...
      if (Cmp == nullptr)
        return EMPTY_BOUNDS;
      return (Cmp->isZero() ? calculateBoundsConstant(DL, CE->getOperand(2))
...
static llvm::Value *narrowBounds(llvm::Module &M, const InsertPoint &IP,
  ...
  if (Entry.bounds == BoundsNonFat)
    ...
    llvm::Value *Bounds = builder.CreateCall(Narrow, {Entry.bounds, SubBounds});
    return Bounds;
  }
}
...
static llvm::Value *narrowBoundsGEP(llvm::Module &M, llvm::Function &F,
                                    ...
                                    const InsertPoint &IP, int idx,
                                    llvm::Value *Bounds, ssize_t lb,
                                    ssize_t ub) {
  ...
  if (idx > 0) {
    ...
    llvm::Value *Ptr =
        builder.CreateGEP(GEP->getPointerOperand(), Idxs, "subptr");
...
static BoundsEntry calculateBoundsGEP(llvm::Module &M, llvm::Function &F,
                                      llvm::GEPOperator *GEP,
                                      const InsertPoint &IP, TypeInfo &tInfo,
                                      CheckInfo &cInfo, BoundsInfo &bInfo) {
  llvm::Value *Ptr = GEP->getPointerOperand();
  llvm::Type *Ty = Ptr->getType();
...
static const BoundsEntry &calculateBounds(llvm::Module &M, llvm::Function &F,
  ...
  if ((option_no_globals && llvm::isa<llvm::Constant>(Ptr)) ||
    ...
    case llvm::Instruction::GetElementPtr: {
      ...
      auto [Bounds1, lb1, ub1] =
          calculateBoundsGEP(M, F, GEPOp, IP, tInfo, cInfo, bInfo);
      Bounds = Bounds1;
      lb = lb1;
      ub = ub1;
      break;
    ...
    case llvm::Instruction::BitCast: {
      ...
      if (PtrTy != nullptr && PtrTy->getElementType()->isIntegerTy(8)) {
        ...
        const BoundsEntry &Entry =
            calculateBounds(M, F, CE->getOperand(0), tInfo, cInfo, bInfo);
        Bounds = Entry.bounds;
        lb = Entry.lb;
        ub = Entry.ub;
        break;
    ...
    case llvm::Instruction::Select: {
      ...
      if (Cmp == nullptr) {
        ...
        llvm::Value *Bounds =
            instrumentTypeCheck(M, F, Ptr, tInfo, cInfo, /*untyped=*/true);
        BoundsEntry Entry = {Bounds, INTPTR_MIN, INTPTR_MAX};
        auto k = bInfo.insert(std::make_pair(Ptr, Entry));
        return k.first->second;
      }
    ...
    if (canIgnoreBitCast(Cast, cInfo, &SubObjTy)) {
      const BoundsEntry &Entry =
          calculateBounds(M, F, Cast->getOperand(0), tInfo, cInfo, bInfo);
      Bounds = Entry.bounds;
      lb = Entry.lb;
      ub = Entry.ub;
      if (SubObjTy != nullptr) {
    ...
    const BoundsEntry &EntryA =
        calculateBounds(M, F, PtrA, tInfo, cInfo, bInfo);
    const BoundsEntry &EntryB =
        calculateBounds(M, F, PtrB, tInfo, cInfo, bInfo);
    if (EntryA.lb == EntryB.lb && EntryA.ub == EntryB.ub) {
      if (EntryA.bounds != EntryB.bounds) {
        llvm::IRBuilder<> builder(Select);
        Bounds = builder.CreateSelect(Select->getOperand(0), EntryA.bounds,
                                      EntryB.bounds);
      }
      lb = EntryA.lb;
      ub = EntryA.ub;
      ...
      llvm::IRBuilder<> builder(Select);
      Bounds = builder.CreateSelect(Select->getOperand(0), BoundsA, BoundsB);
    }
    ...
    for (unsigned i = 0; i < numValues; i++) {
      ...
      const BoundsEntry &EntryIn =
          calculateBounds(M, F, PtrIn, tInfo, cInfo, bInfo);
      ...
      if (!haveSubBounds) {
        ...
        allSameSubBounds =
            allSameSubBounds && (lb1 == EntryIn.lb && ub1 == EntryIn.ub);
    }
    if (allSameSubBounds) {
    ...
    /*NOP*/;
  else if (llvm::isa<llvm::CallInst>(Ptr) || llvm::isa<llvm::InvokeInst>(Ptr) ||
           llvm::isa<llvm::AllocaInst>(Ptr) || llvm::isa<llvm::Argument>(Ptr) ||
...
static bool insertBoundsCheck(const llvm::DataLayout &DL, llvm::Instruction *I,
                              llvm::Value *Ptr, llvm::Value *Ptr0,
                              ssize_t offset, size_t size, llvm::Value *Size,
                              bool escape, const CheckInfo &cInfo,
  ...
  if (size == SIZE_MAX) {
    ...
    if (GEP->accumulateConstantOffset(DL, Offset)) {
      ...
      for (int i = 0; !isOOB && i < numIdxs; i++) {
        ...
        if (Int == nullptr)
          ...
          isOOB = (idx < 0 || idx >= (ssize_t)ArrayTy->getNumElements());
        } else if (auto *StructTy = llvm::dyn_cast<llvm::StructType>(Ty)) {
      ...
      if (!isOOB) {
        ...
        if (insertBoundsCheck(DL, I, GEP->getPointerOperand(), Ptr0, offset,
    ...
    if (canIgnoreBitCast(Cast, cInfo, &SubObjTy)) {
      ...
      if ((SubObjTy == nullptr ||
          ...
          insertBoundsCheck(DL, I, Cast->getOperand(0), Ptr0, offset, size,
                            Size, escape, cInfo, bcInfo)) {
        return true;
      }
    }
  }
  ...
  BoundsCheckEntry Check = {Ptr0,   I,    Ptr,  lb,       ub,
                            offset, size, Size, redundant};
...
static void insertBoundsCheck(const llvm::DataLayout &DL, llvm::Instruction *I,
                              llvm::Value *Ptr, size_t size, llvm::Value *Size,
                              bool escape, const CheckInfo &cInfo,
                              BoundsCheckInfo &bcInfo) {
  bool force = true;
  if (!insertBoundsCheck(DL, I, Ptr, Ptr, 0, size, Size, escape, cInfo, bcInfo,
                         force)) {
 ...
 */
static void
findPointersForBoundsCheck(llvm::Module &M, llvm::Function &F,
  ...
  for (auto &BB : F) {
    for (auto &I : BB) {
      ...
      if (Ptr != nullptr) {
        ...
        insertBoundsCheck(DL, &I, Ptr, size, nullptr, false, cInfo, bcInfo);
      }
      if (auto *MT = llvm::dyn_cast<llvm::MemTransferInst>(&I)) {
        ...
        insertBoundsCheck(DL, &I, Src, size, Size, false, cInfo, bcInfo);
        insertBoundsCheck(DL, &I, Dst, size, Size, false, cInfo, bcInfo);
      ...
      for (auto *Ptr : Ptrs)
        insertBoundsCheck(DL, &I, Ptr, 0, nullptr, true, cInfo, bcInfo);
    }
  }
}
...
instrumentBoundsCheck(llvm::Module &M, llvm::Function &F,
                      const BoundsCheckEntry &Check, TypeInfo &tInfo,
                      CheckInfo &cInfo, BoundsInfo &bInfo) {
  if (Check.redundant)
  ...
  llvm::Constant *BoundsCheck = M.getOrInsertFunction(
      "effective_bounds_check", builder.getVoidTy(), BoundsTy,
 ...
 */
static bool isWithinAllocationBounds(
    const llvm::DataLayout &DL, const CheckInfo &cInfo, llvm::Value *Ptr,
  ...
  if (i != cInfo.end() && i->second.allocType != nullptr) {
    ...
    if (GEP->accumulateConstantOffset(DL, Offset)) {
      ...
      lb += offset;
      ub += offset;
      return isWithinAllocationBounds(DL, cInfo, GEP->getPointerOperand(), lb,
                                      ub, Seen);
    }
    return false;
  } else if (auto *Cast = llvm::dyn_cast<llvm::BitCastInst>(Ptr)) {
    ...
    if (canIgnoreBitCast(Cast, cInfo, &SubObjTy)) {
      if (SubObjTy == nullptr ||
        ...
        return isWithinAllocationBounds(DL, cInfo, Cast->getOperand(0), lb, ub,
                                        Seen);
      }
    }
    return false;
    ...
    size_t numValues = PHI->getNumIncomingValues();
    for (size_t i = 0; i < numValues; i++) {
      if (!isWithinAllocationBounds(DL, cInfo, PHI->getIncomingValue(i), lb, ub,
                                    Seen))
        return false;
    }
    return true;
...
static void optimizeBoundsChecks(llvm::Module &M, llvm::Function &F,
  ...
  for (auto &entry : bcInfo) {
    ...
    for (auto &Check : Checks) {
      ...
      // redundant.
      std::map<llvm::Value *, std::pair<intptr_t, intptr_t>> Seen;
      if (isWithinAllocationBounds(DL, cInfo, Check.widePtr, Check.wideLb,
                                   Check.wideUb, Seen)) {
      ...
      for (auto &Check2 : Checks) {
        if (Check.instr != Check2.instr &&     // different
            ...
            Check2.wideLb <= Check.wideLb && Check2.wideUb >= Check.wideUb &&
            Check2.accessVarSize == Check.accessVarSize &&
            (Check.accessVarSize != nullptr ? Check2.wideLb == Check.wideLb
                                            : true)) {
          Check.redundant = true;
          break;
        }
...
static EFFECTIVE_NOINLINE void
replaceAllocas(llvm::Module &M, llvm::Function &F, TypeInfo &tInfo,
...
static void replaceGlobal(llvm::Module &M, llvm::GlobalVariable &GV,
  ...
  NewName += GV.getName();
  llvm::LLVMContext &Cxt = M.getContext();
  llvm::StructType *NewTy = llvm::StructType::create(
      Cxt, {ObjMetaTy, Ty}, "EFFECTIVE_GLOBAL_WRAPPER", /*isPacked=*/true);
  ...
  llvm::Constant *NewGV1 =
      llvm::ConstantExpr::getInBoundsGetElementPtr(NewTy, NewGV, Idxs);
...
static EFFECTIVE_NOINLINE void replaceGlobals(llvm::Module &M,
                                              TypeInfo &tInfo) {
  std::vector<llvm::GlobalVariable *> Dels;
...
static EFFECTIVE_NOINLINE void emitInstrumentationFunctions(llvm::Module &M) {
  ...
  if (F != nullptr && !debug) {
    ...
    llvm::BasicBlock *Entry = llvm::BasicBlock::Create(M.getContext(), "", F);
    llvm::BasicBlock *Return = llvm::BasicBlock::Create(M.getContext(), "", F);
    llvm::BasicBlock *Error = llvm::BasicBlock::Create(M.getContext(), "", F);

    llvm::Value *Bounds0 = Bounds;
    {
      llvm::IRBuilder<> builder(Entry);
#ifdef EFFECTIVE_FLAG_PROFILE
      llvm::Value *Counter = M.getOrInsertGlobal("effective_num_bounds_checks",
                                                 builder.getInt64Ty());
      ...
      Ptrs = builder.CreateInsertElement(Ptrs, IPtr, builder.getInt32(0));
      Ptrs = builder.CreateInsertElement(Ptrs, IPtr, builder.getInt32(1));
      llvm::Value *Sizes = llvm::UndefValue::get(BoundsTy);
      LB = builder.CreateAdd(LB, builder.getInt64(1));
      Sizes = builder.CreateInsertElement(Sizes, LB, builder.getInt32(0));
      Sizes = builder.CreateInsertElement(Sizes, UB, builder.getInt32(1));
      ...
      llvm::Intrinsic::ID Id = llvm::Intrinsic::getIntrinsicForGCCBuiltin(
          "x86", "__builtin_ia32_pmovmskb128");
      ...
      llvm::MDNode *Weights = mdBuilder.createBranchWeights(2000000000, 1);
    ...
    }
    {
      ...
      llvm::Constant *BoundsErr = M.getOrInsertFunction(
          "effective_bounds_error", builder.getVoidTy(), BoundsTy,
          builder.getInt8PtrTy(), builder.getInt64Ty(), nullptr);
      builder.CreateCall(BoundsErr, {Bounds0, Ptr, Size});
#else /* EFFECTIVE_FLAG_COUNT */
      llvm::Value *CountPtr = M.getOrInsertGlobal("effective_num_bounds_errors",
                                                  builder.getInt64Ty());
#ifdef EFFECTIVE_FLAG_SINGLE_THREADED
      llvm::Value *Count = builder.CreateAlignedLoad(CountPtr, sizeof(size_t));
  ...
  if (F != nullptr) {
    ...
    Cmp = builder.CreateSExt(Cmp, BoundsTy);
    llvm::Value *Mask =
        M.getOrInsertGlobal("EFFECTIVE_BOUNDS_NEG_1_0", BoundsTy);
...
struct EffectiveSan : public llvm::ModulePass {
  ...
  virtual bool runOnModule(llvm::Module &M) {
    ...
    if (option_debug) {
      ...
      fprintf(stderr,
              "|  __/  _|  _|  __/ (__| |_| |\\ V /  __/___) | (_| | | | |\n");
      fprintf(stderr, " \\___|_| |_|  \\___|\\___|\\__|_| \\_/ \\___|____/ "
                      "\\__,_|_| |_|\n");
      fputc('\n', stderr);
    }
    ...
    std::cerr << "New Module" << M.getName().str() << '\n';

    // Ahmad
    basicTypes.insert(
        std::make_pair("EFFECTIVE_INFO_FREE", MY_EFFECTIVE_INFO_FREE));
    basicTypes.insert(
        std::make_pair("EFFECTIVE_INFO_INT8_PTR", MY_EFFECTIVE_INFO_INT8_PTR));
    basicTypes.insert(
        std::make_pair("EFFECTIVE_INFO_INT8", MY_EFFECTIVE_INFO_INT8));
    basicTypes.insert(
        std::make_pair("EFFECTIVE_INFO_INT16", MY_EFFECTIVE_INFO_INT16));
    basicTypes.insert(
        std::make_pair("EFFECTIVE_INFO_INT32", MY_EFFECTIVE_INFO_INT32));
    basicTypes.insert(
        std::make_pair("EFFECTIVE_INFO_INT64", MY_EFFECTIVE_INFO_INT64));
    basicTypes.insert(
        std::make_pair("EFFECTIVE_INFO_INT128", MY_EFFECTIVE_INFO_INT128));
    basicTypes.insert(
        std::make_pair("EFFECTIVE_INFO_FLOAT16", MY_EFFECTIVE_INFO_FLOAT16));
    basicTypes.insert(
        std::make_pair("EFFECTIVE_INFO_FLOAT32", MY_EFFECTIVE_INFO_FLOAT32));
    basicTypes.insert(
        std::make_pair("EFFECTIVE_INFO_FLOAT64", MY_EFFECTIVE_INFO_FLOAT64));
    basicTypes.insert(
        std::make_pair("EFFECTIVE_INFO_FLOAT80", MY_EFFECTIVE_INFO_FLOAT80));
    basicTypes.insert(
        std::make_pair("EFFECTIVE_INFO_FLOAT128", MY_EFFECTIVE_INFO_FLOAT128));

    currentTypeID = 11;
    std::ifstream afile;

    if (is_file_exist(hashFileName)) {
      afile.open(hashFileName);
      std::string line;
      while (getline(afile, line)) {
        ...
        if (myEffectiveInfos.find(eff_info_global_name) !=
          ...
          std::cerr << eff_info_global_name << "\n";
          EFFECTIVE_FATAL_ERROR("Found redundant effective_gloabl_name\n");
        } else {
          // To-change
          tid = std::stoull(strs[1]);
          if (tid > currentTypeID)
            currentTypeID = tid;
          access = (uint64_t)std::stoi(strs[2]);
          name = strs[3];
          size = std::stoi(strs[4]);
          num_entries = std::stoi(strs[5]);
          flags = std::stoi(strs[6]);
          next = strs[7];

          for (auto i = 0; i < num_entries; i++) {
            auto idx = 8 + i * 4;
            std::string global_name = strs[idx];
            uint32_t flags = std::stoi(strs[idx + 1]);
            size_t lb = (size_t)std::stoi(strs[idx + 2]);
            size_t ub = (size_t)std::stoi(strs[idx + 3]);
    ...
    // make a dummy array of TyCheCacheLineEntryTy and pass it
    llvm::ArrayType *DummyTyCheSectionLayoutTy = llvm::ArrayType::get(TyCheCacheLineEntryTy, 0);
    TypeTy = makeTypeMetaType(M, 0, DummyTyCheSectionLayoutTy);

    std::vector<llvm::Type *> Fields;
    Fields.push_back(llvm::Type::getInt8PtrTy(Cxt)); /* name */
    Fields.push_back(llvm::Type::getInt64Ty(Cxt)); /* offset */
    ...
    llvm::Constant *TypeEntryName = llvm::ConstantExpr::getPointerCast(TypeEntryNameGV, llvm::Type::getInt8PtrTy(Cxt));
    Elems.push_back(TypeEntryName);
    Elems.push_back(llvm::ConstantInt::get(llvm::Type::getInt64Ty(Cxt), UINT64_MAX));
...
namespace llvm {
ModulePass *createEffectiveSanPass() { return new EffectiveSan(); }
} // namespace llvm
