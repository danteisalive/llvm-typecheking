...
public:
  ...
  void printDAG(llvm::raw_fd_ostream &typeTreeOut, std::string moduleName) {
    int nodeNumber = 0;
...
void dumpTypeTree(llvm::DIType *TypeEntry, llvm::raw_fd_ostream &typeTreeOut,
                  TypeNode *root);
/*
...
struct EffectiveSan : public llvm::ModulePass {
  ...
  virtual bool runOnModule(llvm::Module &M) {
    ...
    std::string fileName = "/home/ahmad/Desktop/TypeTree/" + M.getSourceFileName() + ".dot";
    ...
    for (auto &TypeEntry : tInfo.infos) {
      dumpTypeTree(llvm::dyn_cast<llvm::DIType>(TypeEntry.first), typeTreeOut,
                   root);
    ...
    root->printDAG(typeTreeOut, moduleName);
    typeTreeOut.flush();
    typeTreeOut.close();
...
void dumpTypeTree(llvm::DIType *TypeEntry, llvm::raw_fd_ostream &typeTreeOut,
  ...
  if (llvm::isa<llvm::DICompositeType>(TypeEntry)) {
    ...
    if (title.empty()) {
      switch (CompositeTy->getTag()) {
      case llvm::dwarf::DW_TAG_structure_type: {
        title = "structure in " + CompositeTy->getFilename().str() + " " +
    ...
    std::cerr << ansi::foreground_yellow
              << "Composite type (after correction) title is " << ansi::reset
              << title << '\n';

    TypeNode *compositeNode = new TypeNode(title);

    // std::cerr << ansi::foreground_red << "Composite Type "
    //           << CompositeTy->getIdentifier().str() << " added to node "
    //           << parent->title << ansi::reset << '\n';
    // parent->PrintPretty("", true, typeTreeOut, true);

    if (!compositeNode->title.empty() &&
      mySet.find(compositeNode->title) != mySet.end()) {
      TypeNode *source = mySet[compositeNode->title];
      std::cerr << ansi::foreground_yellow
                << "already exists. Deep copy is done on "
                << compositeNode->title << " and " << source->title
                << ansi::reset << '\n';

      // compositeNode->deepCopy(source);
      parent->childs.push_back(compositeNode);
    } else {

      std::cerr << ansi::foreground_green
                << "Composite type found. Dumping it:" << ansi::reset << '\n';
      CompositeTy->dump();
      ...
      case llvm::dwarf::DW_TAG_structure_type: {
        ...
        compositeNode->title += " " + CompositeTy->getFilename().str() + " " + std::to_string(CompositeTy->getLine());
        std::cerr << ansi::foreground_green << "Structure found " << ansi::reset
                  << '\n';
        (parent->childs).push_back(compositeNode);

        std::cerr << ansi::foreground_blue
                  << "FILENAME: " << CompositeTy->getFilename().str()
                  << ansi::reset << '\n';
        ...
        for (auto Element : Elements) {
          ...
          if (llvm::isa<llvm::DIType>(Element)) {
            ...
            if (llvm::dyn_cast<llvm::DIDerivedType>(elementTy)) {
              std::cerr << ansi::foreground_green << "Member is found"
                        << ansi::reset << '\n';
              dumpTypeTree(elementTy, typeTreeOut, compositeNode);
            }

            else
              llvm::report_fatal_error(
                  "DW_TAG_structure_type | DW_TAG_class_type Tags with "
                  "non-DIDerivedType for DICompositeType!",
                  false);
          }
        }
        break;
      ...
      case llvm::dwarf::DW_TAG_union_type: {
        // parent->title = "Union";
        if (title.empty())
          compositeNode->title = "Union";
        else
          mySet[compositeNode->title] = compositeNode;
        (parent->childs).push_back(compositeNode);
        std::cerr << ansi::foreground_green << "Dumping Union Type"
                  << ansi::reset << '\n';
        CompositeTy->dump();
        ...
        for (auto Element : Elements) {
          ...
          if (llvm::isa<llvm::DIType>(Element)) {
            ...
            if (llvm::dyn_cast<llvm::DIDerivedType>(elementTy)) {
              std::cerr << ansi::foreground_green << "Union Member is found"
                        << ansi::reset << '\n';
              dumpTypeTree(elementTy, typeTreeOut, compositeNode);
            }
      ...
      case llvm::dwarf::DW_TAG_array_type: {
        // Ahmad
        // mySet[compositeNode->title] = compositeNode;
        compositeNode->title = "Array: ";
        parent->childs.push_back(compositeNode);
        // std::cerr << ansi::foreground_green << "Array type added to parent "
        //           << parent->title << ansi::reset << "\n";
        parent->PrintPretty("", true, typeTreeOut, true);
        auto *baseType =
            llvm::dyn_cast<llvm::DIType>(CompositeTy->getBaseType());

        if (baseType != nullptr) {
          // if (baseType != nullptr &&
          //     !(llvm::isa<llvm::DICompositeType>(baseType))) {
          dumpTypeTree(baseType, typeTreeOut, compositeNode);
        }
        break;
      }
      ...
      case llvm::dwarf::DW_TAG_enumeration_type: {
        // Ahmad
        std::cerr << ansi::foreground_green << "Enumeration Found"
                  << ansi::reset << '\n';
        // mySet[compositeNode->title] = compositeNode;
        compositeNode->title = "enumeration:";
        compositeNode->title += + " " + CompositeTy->getName().str();
        parent->childs.push_back(compositeNode);
        // std::cerr << ansi::foreground_green << "Array type added to parent "
        //           << parent->title << ansi::reset << "\n";
        parent->PrintPretty("", true, typeTreeOut, true);
      ...
      default:
        CompositeTy->dump();
        llvm::report_fatal_error("Unknown Tag for DICompositeType!", false);
      }
    }

    // fprintf(stderr, "FILENAME: %s\n", CompositeTy->getFilename());
    ...
    std::cerr << ansi::foreground_blue
              << "FILENAME: " << DerivedTy->getFilename().str() << ansi::reset
              << '\n';
    switch (DerivedTy->getTag()) {
    ...
    case llvm::dwarf::DW_TAG_pointer_type: {
      ...
      pointerNode->title += DerivedTy->getFilename().str() + " " + std::to_string(DerivedTy->getLine());
      (parent->childs).push_back(pointerNode);
      std::cerr << ansi::foreground_red << "Pointer type added to parent "
                << parent->title << ansi::reset << "\n";
      parent->PrintPretty("", true, typeTreeOut, true);
      DerivedTy->dump();
      std::cerr << ansi::foreground_red << "----------------------------------"
                << ansi::reset << "\n";
      ...
      if (baseType != nullptr) {
        ...
        // if (baseType != nullptr &&
        //     !(llvm::isa<llvm::DICompositeType>(baseType))) {
        dumpTypeTree(baseType, typeTreeOut, pointerNode);
      }
      break;
    }
    case llvm::dwarf::DW_TAG_member: {
      ...
      derivedNode->title += " " + DerivedTy->getFilename().str() + " " + std::to_string(DerivedTy->getLine());
      (parent->childs).push_back(derivedNode);

      // std::cerr << ansi::foreground_green << "Member type  "
      //           << DerivedTy->getName().str() << " is added to type "
      //           << parent->title << ansi::reset << "\n";
      ...
      // Ahmad
      if (baseType != nullptr)
        dumpTypeTree(baseType, typeTreeOut, derivedNode);
      else
        std::cerr << ansi::foreground_red << "Found null pointer "
                  << ansi::reset << '\n';

      break;
    }
    case llvm::dwarf::DW_TAG_typedef: {
      std::cerr << ansi::foreground_yellow << "Found typedef" << ansi::reset
                << '\n';
      TypeNode *typeDefNode = new TypeNode(DerivedTy->getName().str());
      typeDefNode->title += DerivedTy->getFilename().str() + " " + std::to_string(DerivedTy->getLine());
      (parent->childs).push_back(typeDefNode);

      auto *baseType = llvm::dyn_cast<llvm::DIType>(DerivedTy->getBaseType());

      if (baseType != nullptr) {
        dumpTypeTree(baseType, typeTreeOut, typeDefNode);
      }
      break;
    }

    case llvm::dwarf::DW_TAG_inheritance: {
      TypeNode *pointerNode = new TypeNode("Inherited Type");
      (parent->childs).push_back(pointerNode);
      std::cerr << ansi::foreground_red << "Inherited type added to parent "
                << parent->title << ansi::reset << "\n";
      parent->PrintPretty("", true, typeTreeOut, true);
      DerivedTy->dump();
      std::cerr << ansi::foreground_red << "----------------------------------"
    ...
    case llvm::dwarf::DW_TAG_const_type: {
      TypeNode *pointerNode = new TypeNode("Const Type");
      (parent->childs).push_back(pointerNode);
      std::cerr << ansi::foreground_red << "Constant type added to parent "
                << parent->title << ansi::reset << "\n";
      parent->PrintPretty("", true, typeTreeOut, true);
      DerivedTy->dump();
      std::cerr << ansi::foreground_red << "----------------------------------"
    ...
    case llvm::dwarf::DW_TAG_friend:
      // DerivedTy->dump();
      // llvm::report_fatal_error("DW_TAG_inheritance Tag for DIDerivedType!",
      //                          false);
      // break;

    case llvm::dwarf::DW_TAG_reference_type: {
      TypeNode *referenceNode = new TypeNode("some reference");
      (parent->childs).push_back(referenceNode);
      std::cerr << ansi::foreground_red << "Reference type added to parent "
                << parent->title << ansi::reset << "\n";
      parent->PrintPretty("", true, typeTreeOut, true);
      DerivedTy->dump();
      std::cerr << ansi::foreground_red << "----------------------------------"
      ...
      auto *baseType = llvm::dyn_cast<llvm::DIType>(DerivedTy->getBaseType());
      if (baseType != nullptr) {
        dumpTypeTree(baseType, typeTreeOut, referenceNode);
      }
      break;
    }
    case llvm::dwarf::DW_TAG_ptr_to_member_type: {
      TypeNode *pointerToMemberNode = new TypeNode("Pointer to member");
      parent->childs.push_back(pointerToMemberNode);
      auto *baseType = llvm::dyn_cast<llvm::DIType>(DerivedTy->getBaseType());
      dumpTypeTree(baseType, typeTreeOut, pointerToMemberNode);
      break;
    }

    case llvm::dwarf::DW_TAG_volatile_type: {
      TypeNode *volatileNode = new TypeNode("Volatile type");
      parent->childs.push_back(volatileNode);
      auto *baseType = llvm::dyn_cast<llvm::DIType>(DerivedTy->getBaseType());
      dumpTypeTree(baseType, typeTreeOut, volatileNode);
      break;
    }
    case llvm::dwarf::DW_TAG_restrict_type:
    case llvm::dwarf::DW_TAG_atomic_type:
      // DerivedTy->dump();
      // llvm::report_fatal_error("DW_TAG_reference_type Tag for
      // DIDerivedType!",
      //                          false);
      // break;
    ...
    default:
      DerivedTy->dump();
      llvm::report_fatal_error("error, Unknown Tag for DIDerivedType!", false);
    }
    ...
    TypeNode *basicNode;
    std::cerr << ansi::foreground_blue
              << "FILENAME: " << basicType->getFilename().str() << ansi::reset
              << '\n';
    ...
    case llvm::dwarf::DW_TAG_base_type:
      ...
      // parent->name = basicType->getName().str() + " *";
      std::cerr << ansi::foreground_red << "Basic type " << basicNode->title
                << " added to parent " << parent->title << ansi::reset << "\n";
      parent->PrintPretty("", true, typeTreeOut, true);
      basicType->dump();
      std::cerr << ansi::foreground_red << "----------------------------------"
                << ansi::reset << "\n";

      // fprintf(stderr, "Basic Type Added\n");

      // if (mySet.find(basicType->getName().str()) == mySet.end()) {
      //   typeTreeOut << "New type added \"" << basicType->getName() <<
      //   "\"\n";
      //   // mySet.insert(basicType->getName().str());
      // }
    ...
    subroutineNode->title += subroutineType->getFilename().str() + " " + std::to_string(subroutineType->getLine()) + " " + subroutineType->getName().str();
    parent->childs.push_back(subroutineNode);

    // llvm::DITypeRefArray Types = subroutineType->getTypeArray();
    // for (auto Arg : Types) {
    //   llvm::DIType *ArgTy = Arg.resolve();
    //   if (ArgTy != nullptr) {
    //     ArgTy->dump();
    //     dumpTypeTree(ArgTy, typeTreeOut, subroutineType);
    //   }
    //   // ArgTy->dump();
    // }

    // llvm::report_fatal_error("DISubroutineType in Type Tree!", false);
  } else {
    TypeEntry->dump();
    std::cerr << ansi::foreground_red << "Unknown Type in Type Tree!"
              << ansi::reset << "\n";
    llvm::report_fatal_error("Unknown Type in Type Tree!", false);
  }
