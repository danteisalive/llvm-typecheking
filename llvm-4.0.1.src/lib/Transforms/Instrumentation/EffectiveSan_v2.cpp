...
public:
  ...
  void printDAG(llvm::raw_fd_ostream &typeTreeOut) {
    int nodeNumber = 0;
...
struct EffectiveSan : public llvm::ModulePass {
  ...
  virtual bool runOnModule(llvm::Module &M) {
    ...
    std::string fileName =
        "/home/ahmad/Desktop/TypeTree/" + CurrentDate() + ".dot";
...
void dumpTypeTree(llvm::DIType *TypeEntry, llvm::raw_fd_ostream &typeTreeOut,
  ...
  if (llvm::isa<llvm::DICompositeType>(TypeEntry)) {
    ...
    if (title.empty()) {
      switch (CompositeTy->getTag()) {
      case llvm::dwarf::DW_TAG_structure_type: {
        title = "Struct: in " + CompositeTy->getFilename().str() + " " +
    ...
    case llvm::dwarf::DW_TAG_typedef: {
      std::cerr << ansi::foreground_yellow << "Found typedef" << ansi::reset
                << '\n';
      TypeNode *typeDefNode = new TypeNode("Alias: " + DerivedTy->getName().str());
    ...
    case llvm::dwarf::DW_TAG_const_type: {
      TypeNode *pointerNode = new TypeNode("Const: ");
      (parent->childs).push_back(pointerNode);
    ...
    case llvm::dwarf::DW_TAG_reference_type: {
      TypeNode *referenceNode = new TypeNode("Reference: ");
    ...
    case llvm::dwarf::DW_TAG_volatile_type: {
      TypeNode *volatileNode = new TypeNode("Volatile: ");
