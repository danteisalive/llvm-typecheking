...
#     MOUNT_POINT=$PWD/mnt
#     mkdir -p "$MOUNT_POINT"
#     SPEC2006_ISO=$PWD/cpu2006-1.1.iso
#     if [ -e "$SPEC2006_ISO" ]
#     then
...
#         echo -e "${GREEN}log${OFF}: attempting to install SPEC2006 from" \
#             "/dev/cdrom..."
#         sudo mount --read-only $SPEC2006_ISO "$MOUNT_POINT"
#     fi
...
echo "runspec -c $NAME -a run -I -l --size $SIZE -n $NUM_RUNS $@"
echo $PWD
