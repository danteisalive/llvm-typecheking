...
static llvm::StructType *TidTy       = nullptr;
...
static llvm::StructType *makeTypeInfoType(llvm::Module &M, size_t len)
{
    ...
    if (len == 0){
        InfoTy = Ty;
        TidTy = llvm::StructType::create(Cxt, "EFFECTIVE_TID");
        //llvm::StructType *TidTy_t = llvm::StructType::create(Cxt, TidName);
        std::vector<llvm::Type *> Fields;
        Fields.push_back(llvm::Type::getInt64Ty(Cxt));  
        Fields.push_back(llvm::Type::getInt64Ty(Cxt));    
        TidTy->setBody(Fields, false);
    }


    std::vector<llvm::Type *> Fields;
    Fields.push_back(TidTy->getPointerTo());            /* tid_info */
    ...
    size_t size, const FAM &fam, bool incomplete, TypeInfo &tInfo)
{
    ...
    std::stringstream TidInfoName;
    TidInfoName << TYPE_TID_INFO_PREFIX;
    TidInfoName << std::hex << hash.i64[1] << hash.i64[0];
    ...
    llvm::Constant *Name = llvm::ConstantExpr::getPointerCast(NameGV, llvm::Type::getInt8PtrTy(Cxt));
...
/////////////////////////////////////////////////////////////////////////////
    // std::string TidName("EFFECTIVE_TID");
    // TidName += '_';
    // TidName += std::to_string(Entries.size());

    // llvm::StructType *TidTy_t = llvm::StructType::create(Cxt, TidName);
    // std::vector<llvm::Type *> Fields;
    // Fields.push_back(llvm::Type::getInt64Ty(Cxt));  
    // Fields.push_back(llvm::Type::getInt64Ty(Cxt));    
    // TidTy_t->setBody(Fields, false);

    llvm::GlobalVariable *TidInfoGV = new llvm::GlobalVariable(M, TidTy, false, llvm::GlobalValue::WeakAnyLinkage, 0, TidInfoName.str());
    llvm::Constant *Tid_t = llvm::ConstantInt::get(llvm::Type::getInt64Ty(Cxt), 120);
    llvm::Constant *NumAccesses_t = llvm::ConstantInt::get(llvm::Type::getInt64Ty(Cxt), 120);
    llvm::Constant *TidInfoInit = llvm::ConstantStruct::get(TidTy,{Tid_t, NumAccesses_t});
    TidInfoGV->setInitializer(TidInfoInit);
    llvm::Constant *TidInfo = llvm::ConstantExpr::getBitCast(TidInfoGV, TidTy->getPointerTo());
////////////////////////////////////////////////////////////////////////////

    llvm::StructType *InfoTy1 = makeTypeInfoType(M, Entries.size());
    
    llvm::GlobalVariable *InfoGV = new llvm::GlobalVariable(M, InfoTy1, true, llvm::GlobalValue::WeakAnyLinkage, 0, infoName.str());
    llvm::Constant *Size = llvm::ConstantInt::get(llvm::Type::getInt32Ty(Cxt), size);
    llvm::Constant *NumEntries = llvm::ConstantInt::get(llvm::Type::getInt32Ty(Cxt), Entries.size());
    llvm::Constant *Flags = llvm::ConstantInt::get(llvm::Type::getInt32Ty(Cxt), flags);
    ...
    llvm::ArrayType *EntriesTy = llvm::ArrayType::get(InfoEntryTy, Entries.size());
    llvm::Constant *Entries1 = llvm::ConstantArray::get(EntriesTy, Entries);

    llvm::Constant *InfoInit = llvm::ConstantStruct::get(InfoTy1,{ TidInfo, Name, Size, NumEntries, Flags, Next, Entries1});
    InfoGV->setInitializer(InfoInit);
    llvm::Constant *Info = llvm::ConstantExpr::getBitCast(InfoGV, InfoTy->getPointerTo());
