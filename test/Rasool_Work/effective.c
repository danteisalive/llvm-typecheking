    ...
    const EFFECTIVE_TYPE *u)
{
    ...
    if (u->info->tid_info != NULL && t->info->tid_info != NULL)
        EFFECTIVE_DEBUG("effective_type_check(SUB_TID(%d), TID(%d), %p, %s, %s (%+zd)) = ", u->info->tid_info->num_accesses, t->info->tid_info->num_accesses ,ptr,
            u->info->name, t->info->name, (ssize_t)offset);
    else
    {
        EFFECTIVE_DEBUG("effective_type_check( %p, %s, %s (%+zd)) = " ,ptr, u->info->name, t->info->name, (ssize_t)offset);
    }
    
    
    //t->info->tid_info->num_accesses++;
    //u->info->tid_info->num_accesses++;
